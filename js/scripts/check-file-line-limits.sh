...
if [ -f "$RELEASE_YML" ]; then
  ...
  if [ "$line_count" -gt "$LIMIT" ]; then
    ...
    echo "::error file=$RELEASE_YML::File has $line_count lines (limit: ${LIMIT}). Move inline scripts to ./js/scripts/ folder."
...
if [ "${#FAILURES[@]}" -gt 0 ]; then
  ...
  printf '  %s\n' "${FAILURES[@]}"
  echo ""
  echo "Move large inline scripts to the ./js/scripts/ folder to reduce file size."
  exit 1
else
