...
async function main() {
  try {
    ...
    if (mode === 'instant') {
      ...
      if (description) {
        await $`node js/scripts/instant-version-bump.mjs --bump-type ${bumpType} --description ${description} --js-root ${jsRoot}`;
      } else {
        await $`node js/scripts/instant-version-bump.mjs --bump-type ${bumpType} --js-root ${jsRoot}`;
      }
    } else {
