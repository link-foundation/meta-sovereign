...
# modules and `rust/target/doc/` for the Rust crates) and upload them as
# release assets to the GitHub Release identified by the supplied tag.
#
# Usage: bash js/scripts/attach-api-docs.sh <tag>
...
if [ -z "$TAG" ]; then
  ...
  echo "Usage: bash js/scripts/attach-api-docs.sh <tag>" >&2
  exit 1
fi
...
if [ -d rust/target/doc ]; then
  tar -czf "$RUST_DOCS_TARBALL" -C rust/target doc
  echo "Bundled $RUST_DOCS_TARBALL"
else
  echo "WARNING: rust/target/doc/ not present, skipping Rust docs upload" >&2
fi
