...
// (built by `cargo build --manifest-path rust/Cargo.toml --release -p meta-sovereign-server`). The
...
// Rust backend: spawn `meta-sovereign-rs serve --web js/src/web --port 0`
// and wait for it to print its bound port. Uses release build if
// available, otherwise debug.
const findRustBinary = async () => {
  const candidates = [
    path.join(repoRoot, 'rust', 'target', 'release', 'meta-sovereign-rs'),
    path.join(repoRoot, 'rust', 'target', 'debug', 'meta-sovereign-rs'),
  ];
...
const startRustBackend = async () => {
  ...
  const webDir = path.join(repoRoot, 'js', 'src', 'web');
  ...
  if (!rustBackend) {
    console.log(
      '[e2e-browser-spa] SKIP rust: meta-sovereign-rs binary not found ' +
        '(run `cargo build --manifest-path rust/Cargo.toml -p meta-sovereign-server` first)'
    );
  } else {
    try {
