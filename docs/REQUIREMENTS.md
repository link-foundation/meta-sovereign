...
| R-H4 | Test coverage spans unit, integration, and e2e (`browser-commander` for e2e). | Partial: 118 JS + 49 Rust tests; e2e via `browser-commander` is contract-stable but uses fetch shim until the package lands. |
...
| R-J7  | All critical UI paths covered by e2e tests using [`browser-commander`](https://github.com/link-foundation/browser-commander).             | Partial: contract-stable e2e harness (`tests/browser-commander.test.js`) that swaps to `bc.click` once the package lands; tracked on the roadmap.                                            |
