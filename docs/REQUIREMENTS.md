...
| ID   | Requirement                             | State                                                                   |
| ---- | --------------------------------------- | ----------------------------------------------------------------------- |
| R-E1 | Native support for **VK**.              | Skeleton: archive parser; live API requires `vk-bot` integration.       |
| R-E2 | Native support for **Telegram**.        | Skeleton: archive parser; live API requires `telegram-bot` integration. |
| R-E3 | Native support for **X**.               | Skeleton: archive parser.                                               |
| R-E4 | Native support for **WhatsApp**.        | Skeleton: per-chat export parser.                                       |
| R-E5 | Native support for **Facebook**.        | Skeleton: download-your-data parser.                                    |
| R-E6 | Native support for **LinkedIn**.        | Skeleton: data-export parser.                                           |
| R-E7 | Native support for **career.habr.com**. | Skeleton: resume + applications.                                        |
| R-E8 | Native support for **hh.ru**.           | Skeleton: resume + applications.                                        |
| R-E9 | Native support for **superjob.ru**.     | Skeleton: resume + applications.                                        |
...
| R-F8 | Universal Links access interface in both server and client.                     | Done: server = `DualStore`; client = `BrowserStore` ⊕ `OfflineClient` (server-preferred).  |

## G. Stack constraints

| ID   | Requirement                                                                            | State                                                                                                                     |
| ---- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| R-G1 | Default stack: JS on server + client + React for UI + Rust + WASM for heavy workloads. | Partial: SPA is currently vanilla JS; a React port is on the roadmap.                                                     |
...
| ID   | Requirement                                                                   | State                                                                                                                                                                                                                                                                                                           |
| ---- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R-H1 | UI follows Apple HIG / Google Material / Microsoft design best practices.     | Done: SPA uses system fonts, large tap targets, semantic landmarks (`<header>`/`<nav>`/`<main>`), skip-link, dark-mode toggle, and `aria-pressed` / `aria-label` on stateful controls; the e2e suite runs an axe-core WCAG 2.0 A/AA audit on every backend boot and fails on any serious or critical violation. |
...
| R-H4 | Test coverage spans unit, integration, and e2e (`browser-commander` for e2e). | Done: 129 JS + 52 Rust tests; real-browser e2e via `browser-commander` + Playwright in `tests/e2e-browser-spa.mjs` runs against the JS server and (with `RUN_BROWSER_E2E_RUST=1`) the pure-Rust `meta-sovereign-rs serve` binary, asserting wire-protocol parity end-to-end.                                    |
...
| ID    | Requirement                                                                                                                               | State                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R-J1  | The SPA must work fully offline using browser storage (in-memory / `localStorage` / `IndexedDB`).                                         | Done: `pickBrowserDriver()` selects the best available backend.                                                                                                                                                                                                                                                                                                                                                                                     |
...
| R-J7  | All critical UI paths covered by e2e tests using [`browser-commander`](https://github.com/link-foundation/browser-commander).             | Done: `tests/e2e-browser-spa.mjs` drives headless Chromium via `browser-commander` + Playwright across the reachable critical UI paths (nav, write→reload, pattern infer→save, automation graph, broadcast, outreach, backup→restore, profile sync envelopes, theme, axe-core WCAG 2.0 A/AA audit, skip-link a11y) on the JS server, and re-runs the protocol-parity subset against `meta-sovereign-rs serve` when `RUN_BROWSER_E2E_RUST=1` is set. |
