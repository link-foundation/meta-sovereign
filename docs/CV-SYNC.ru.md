...
Оба сегмента этого пути записаны так, чтобы их приняла любая файловая
система: run id выглядит как `linkedin-2026-09-16T07-00-00.000Z`, а не
как ISO-метка времени, потому что Windows считает `:` в пути
разделителем alternate data stream и вовсе не создаёт каталог.
